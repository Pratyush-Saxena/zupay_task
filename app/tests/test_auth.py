...
def test_auth_login_before_signup():
    ...
    resp=req.get(test.host+'/auth/login',params={'email':test.email,'password':test.password})
    assert resp.status_code==401

def test_auth_signup():
    """Test signup"""
    resp=req.post(test.host+'/auth/signup',json={'email':test.email,'password':test.password})
