...
@router.get('/')
async def all_posts(db: DatabaseManager = Depends(get_database),token: str = Depends(oauth2)):
...
@router.put('/')
async def update_post(post: PostSchema, db: DatabaseManager = Depends(get_database),token: str = Depends(oauth2)):
        if not (token and verify_token(token)):
                return JSONResponse(status_code=401, content={"message": "Unauthorized"})
        user = get_user_id(token)
...
async def add_post(post_response: PostSchema, db: DatabaseManager = Depends(get_database),token: str = Depends(oauth2)):
        if  not (token and verify_token(token)):
                return JSONResponse(status_code=401, content={"message": "Unauthorized"})
        user = get_user_id(token)
...
@router.delete('/')
async def delete_post(post_id: OID, db: DatabaseManager = Depends(get_database),token: str = Depends(oauth2)):
        if  not (token and verify_token(token)):
                return JSONResponse(status_code=401, content={"message": "Unauthorized"})
        user = get_user_id(token)
