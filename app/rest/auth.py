...
@router.get('/login')
async def login(email:EmailStr,password:str , db: DatabaseManager = Depends(get_database)):
    id=await db.get_user(email,password)
    if id:
        token = signJWT(id)
        return JSONResponse(status_code=200, content=token)
    return JSONResponse(status_code=401, content={"message": "Invalid credentials or user does not exist"})

@router.get('/remove_user')
async def remove_user(email:str,admin_secret_key:str ,db: DatabaseManager = Depends(get_database)):
